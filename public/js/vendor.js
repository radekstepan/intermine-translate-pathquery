...
require.define({
  'translate': function(exports, require, module) {
    var root;
    root = this;
    module.exports.toXML = function(json, xml) {
      assert(root.intermine != null, 'Library `imjs` not found');
      try {
        if (typeof json === 'string') {
          json = JSON.parse(json);
        }
        return xml(null, (new root.intermine.Query(json)).toXML());
      } catch (error) {
        return xml(error);
      }
    };
    return module.exports.toJSON = function(xml, json) {
      var attributes, query, sax;
      assert(root.sax != null, 'Library `sax-js` not found');
      sax = root.sax.parser(true);
      query = {};
      attributes = {};
      sax.onattribute = function(attr) {
        switch (attr.name) {
          case 'view':
            return attributes['select'] = attr.value.split(' ');
          default:
      ...
      sax.onopentag = function(node) {
        ...
        switch (node.name) {
          case 'query':
            ...
            break;
          case 'constraint':
            if ((_ref = query['constraints']) == null) {
              query['constraints'] = [];
            }
            query['constraints'].push(attributes);
            break;
          case 'join':
            if ((_ref1 = query['joins']) == null) {
              query['joins'] = [];
            }
      ...
      sax.onend = function() {
        return json(null, JSON.stringify(query, null, 4));
      };
      sax.onerror = function(e) {
        return json(e);
      };
      return sax.write(xml).close();
    };
  }
});
;
